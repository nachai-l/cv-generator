...
#   export ID_TOKEN="<pre-generated-token>"
DEFAULT_SERVICE_URL = (
    "https://cv-generation-service-810737581373.asia-southeast1.run.app"
)

SERVICE_URL = os.getenv("SERVICE_URL", DEFAULT_SERVICE_URL)
API_URL = f"{SERVICE_URL.rstrip('/')}/generate_cv"


def get_id_token() -> str:
    """
    Get an ID token for calling Cloud Run.

    Priority:
      1) Use env var ID_TOKEN if set (fast, good for CI)
      2) Else call `gcloud auth print-identity-token`
    """
    env_token = os.getenv("ID_TOKEN")
    if env_token:
    ...
    try:
        ...
        print(f"❌ Failed to obtain ID token via gcloud: {exc}", file=sys.stderr)
        raise
...
def main() -> None:
    print("\n" + "=" * 80)
    print("🧪  RUNNING REAL API CV GENERATION (v4 • Cloud Run)")
    print("=" * 80)
    ...
    # Get ID token once and reuse
    print("[2.5] Obtaining ID token via gcloud (or env ID_TOKEN) ...")
    ...
    for name, payload in testcases.items():
        ...
        # 3B. Call REAL Cloud Run API to get WITH-LLM response
        try:
